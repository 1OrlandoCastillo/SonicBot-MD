...
case "menuaudio": {
  try {
    ...
    let listaMensaje = `🎧 𓆩 𝐌𝐄𝐍𝐔́ 𝐌𝐔𝐋𝐓𝐈𝐌𝐄𝐃𝐈𝐀 — SONIC 𝟐.𝟎 𓆪
...
🚀 *Sonic 2.0 Bot está en constante expansión.*
👨‍💻 *Desarrollado por Orlando OFF*`;

    await sock.sendMessage2(
      msg.key.remoteJid,
      {
        image: { url: "https://qu.ax/OurJy.jpg" },
...
🎶 *Sonic esta enviando tu música...*`.trim();
...
⏳ *Espera un momento...*  
⚙️ *Sonic Ultra 2.0 está procesando tu música...*

═════════════════════  
     𖥔 SONIC 𝗨𝗹𝘁𝗋𝗮 2.0 BOT 𖥔
═════════════════════`;
...
⏳ *Espera un momento...*  
⚙️ *Sonic 2.0 está procesando tu video...*

═════════════════════  
     𖥔 Sonic 𝗨𝗹𝘁𝗋𝗮 2.0 BOT𖥔
═════════════════════`;
...
case 'robar': {
  try {
    ...
    if (!rpgData.usuarios[userId]) {
      return sock.sendMessage(msg.key.remoteJid, {
        text: `❌ *No tienes una cuenta registrada en el gremio SonicBot.*\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.`
      }, { quoted: msg });
    }
...
case 'gremio': {
    try {
        ...
        if (!rpgData.usuarios || Object.keys(rpgData.usuarios).length === 0) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: "📜 *No hay miembros registrados en el Gremio SonicBot.*\nUsa `" + global.prefix + "rpg <nombre> <edad>` para unirte." 
            }, { quoted: msg });
            return;
        }
...
await sock.sendMessage(msg.key.remoteJid, { 
    image: { url: "https://qu.ax/nkYhc.jpg" }, 
  ...
  break;
    ...
    try {
        ...
        const caption = `*Información de TikTok:*\n\n` +
                        ...
                        `✨ *Información obtenida por SonicBot Ultra 2.0 Bot*`;
...
case 'allmenu': {
  try {
    ...
    let commandList = `📚 𓆩 𝐌𝐄𝐍𝐔́ 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐎 - SONIC 𝟐.𝟎 𝐁𝐎𝐓 𓆪
...
case 'menuowner': {
  try {
    ...
    const chatId = msg.key.remoteJid;
    const captionText = `👑 SONIC 2.0 — PANEL DEL CREADOR
...
👨‍💻 Dev: Orlando OFF`;
    ...
    await sock.sendMessage2(
      chatId,
      {
        image: { url: "https://qu.ax/TzmFV.jpg" },
        caption: captionText
      },
      msg
    );
...
case 'menurpg': {
  try {
    ...
    const chatId = msg.key.remoteJid;
    const menuText = `🎮 𓆩 SONIC 𝟐.𝟎 𝐁𝐎𝐓 — 𝐌𝐄𝐍𝐔́ 𝐑𝐏𝐆 𓆪
...
🧙 *Desarrollado por:* Orlando OFF  
🧭 *Sistema de aventura activo en Sonic 2.0 Bot*`;
...
╭────────────────╮
✨ SONIC 2.0 está en constante mejora.  
...
case 'menugrupo': {
  try {
    ...
    const chatId = msg.key.remoteJid;
    const captionText = `╭─❍ SONIC 𝟐.𝟎 𝐁𝐎𝐓 ❍─╮
│ 🎭 𝙈𝙀𝙉𝙐́ 𝘿𝙀 𝙂𝙍𝙐𝙋𝙊 🎭
╰───────────────╯
...
╰──────────────────╯`;
    ...
    await sock.sendMessage2(
      chatId,
      {
        image: { url: "https://qu.ax/CBrCz.jpg" },
        caption: captionText
      },
      msg
    );
...
case 'todos': {
  try {
    ...
    finalMsg += `╟🔹 *SONIC 2.0 BOT PRESENTE*\n`;
    ...
    await sock.sendMessage(chatId, {
      image: { url: "https://qu.ax/loBiG.jpg" },
      caption: finalMsg,
      mentions: mentionIds
    }, { quoted: msg });
...
case 'cofre': {
    try {
        ...
        if (!rpgData.usuarios[userId]) {
            return sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *No tienes una cuenta registrada en el gremio Sonic 2.0.*\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.` 
            }, { quoted: msg });
        }
        let usuario = rpgData.usuarios[userId];
...
case 'batallauser': {
  try {
    ...
    if (!rpgData.usuarios[userId]) {
      return sock.sendMessage(msg.key.remoteJid, {
        text: `❌ *No tienes una cuenta en el gremio Sonic. Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.*`
      }, { quoted: msg });
    }
    let usuario = rpgData.usuarios[userId];
...
case 'verper': { 
    try { 
        ...
        if (!rpgData.usuarios[userId]) { 
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *No estás registrado en el gremio Sonic 2.0.*\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.` 
            }, { quoted: msg });
            return; 
        }
        ...
        let mensaje = `🎭 *Tus Personajes Comprados - Sonic 2.0* 🎭\n\n`;
    ...
    try {
        ...
        if (!fs.existsSync(rpgFile)) {
            await sock.sendMessage(
                msg.key.remoteJid,
                {
                    text: `❌ *No tienes una cuenta en el gremio Sonic 2.0.*\n\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.`
                },
                { quoted: msg }
            );
            return;
...
case 'nivelper': {
    try {
        ...
        if (!rpgData.usuarios[msg.key.participant]) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *No tienes cuenta en Sonic.*\n📌 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.` 
            }, { quoted: msg });
            return;
        }
...
case 'topuser': {
    try {
        ...
        if (!rpgData.usuarios || Object.keys(rpgData.usuarios).length === 0) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: "📜 *No hay miembros registrados en el Gremio Sonic 2.0.*" 
            }, { quoted: msg });
            return;
        }
        ...
        let ranking = `🏆 *rαทкiทg ∂є jυgα∂σrєs ∂єℓ grємiσ Sonic 2.0* 🏆\n\n`;
        let mentions = [];
   ...
   🐾 *Mascotas:* ${cantidadMascotas}\n\n`;
        ...
        await sock.sendMessage(msg.key.remoteJid, { 
            image: { url: "https://qu.ax/ZjFAz.jpg" },
            caption: ranking,
...
case 'alaventa': {
    try {
        ...
        let mensaje = `🏪 *Mercado de Personajes - Sonic 2.0* 🏪\n\n`;
...
// 📷 Enviar el mensaje con imagen  
await sock.sendMessage(msg.key.remoteJid, { 
    image: { url: "https://qu.ax/kVTGC.jpg" }, 
    caption: mensaje, 
    ...
    try {
        ...
        if (!fs.existsSync(rpgFile)) {
            await sock.sendMessage(
                msg.key.remoteJid,
                {
                    text: `❌ *No tienes una cuenta en el gremio Sonic.*\n\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.`
                },
                { quoted: msg }
            );
            return;
...
🚀 ¡Prepárate para la aventura en *Sonic 2.0*! 🏆`;
...
case 'vermascotas': {  
    try {  
        ...
        let mensaje = ` 🐾 *Tus Mascotas - Sonic 2.0* 🐾\n\n`;  
        ...
        mensaje += `🚀 **¡Sigue entrenando a tus mascotas en el Gremio Sonic!** 🏆`;  
...
await sock.sendMessage(msg.key.remoteJid, {  
 ...
 case 'comprar': {
    try {
        ...
        if (!rpgData.usuarios[userId]) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *No estás registrado en el gremio Sonic.*\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.`
            }, { quoted: msg });
            return;
        }
...
case 'dar': {
    try {
        ...
        if (!rpgData.usuarios[targetUser]) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *El usuario no tiene una cuenta en el gremio Sonic.*\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarlo.` 
            }, { quoted: msg });
            return;
        }
...
case 'deleteuser': {
    try {
        ...
        await sock.sendMessage(msg.key.remoteJid, {
            text: `🗑️ *La cuenta de @${text} ha sido eliminada exitosamente del gremio Sonic 2.0.*\n\n` +
...
case 'deleterpg': {
    try {
        ...
        if (!rpgData.usuarios[userId]) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: `❌ *No tienes un registro en el gremio Sonic 2.0.*\n\n📜 Usa \`${global.prefix}rpg <nombre> <edad>\` para registrarte.` 
            }, { quoted: msg });
            return;
        }
...
case 'ok': {
    try {
        ...
        if (!rpgData.usuarios[userId]) {
            await sock.sendMessage(msg.key.remoteJid, { 
                text: "❌ *No tienes un registro en el gremio Sonic.*" 
            }, { quoted: msg });
            return;
        }
...
await sock.sendMessage(msg.key.remoteJid, { 
    image: { url: "https://qu.ax/FrDRi.jpg" },
    caption: mensaje,
...
case 'tiendamascotas': {
    try {
        ...
        let mensaje = `🏪 *Եíҽղժɑ ժҽ ʍɑՏϲօԵɑՏ - Sonic 𝟐.𝟎* 🏪\n\n`;
        ...
        // Enviar mensaje con imagen estática 🖼️
await sock.sendMessage(msg.key.remoteJid, { 
    image: { url: "https://qu.ax/FrDRi.jpg" }, // Convertido a imagen
    caption: mensaje
}, { quoted: msg });
...
case 'tiendaper': {
    try {
        ...
        let mensaje = `🏪 *ᴛɪᴇɴᴅᴀ ᴅᴇ ᴘᴇʀsᴏɴᴀᴊᴇs - Sonic 𝟸.𝟶* 🏪\n\n`;
        ...
        // Enviar mensaje con imagen estática 🖼️
await sock.sendMessage(msg.key.remoteJid, { 
    image: { url: "https://qu.ax/AvOrt.jpg" }, // Ahora como imagen
    caption: mensaje
}, { quoted: msg });
...
case "s":
    try {
        ...
        let metadata = {
            ...
            author: `🤖 Bot Creador: Sonic 2.0\n🛠️ Desarrollado por: White444\n${fechaCreacion}`
        };
...
case 'info':
  try {
    ...
    const infoMessage = `╭─ *🤖 SONIC 2.0 BOT* ─╮
│ 🔹 *Prefijo actual:* ${global.prefix}
│ 👑 *Dueño:* Orlando OFF
│ 🛠️ *Bot desarrollado desde cero* con la ayuda de Chatgpt.
│ 🚀 *Creado por:* Orlando OFF
│  
├─〔 📥 *Descargas Redes* 〕─
│ 📌 *IG, TikTok y FB*  
│    - 👤 *Colaboró:* DIEGO-OFC  
...
╰──────────────────╯`;
  ...
  {
    image: { url: "https://qu.ax/AvOrt.jpg" }, 
    caption: infoMessage 
  },
  msg 
);
...
case "ping":
    try {
        ...
        await sock.sendMessage(msg.key.remoteJid, {
            image: { url: "https://qu.ax/pBnbJ.jpg" }, 
...
case "perfil": {
    try {
        ...
        try {
            ...
            ppUrl = "https://qu.ax/pBnbJ.jpg"; // Imagen de perfil por defecto
        }
...
case 'creador': {
    const ownerNumber = "2731590195@s.whatsapp.net"; // Número del dueño en formato WhatsApp
    const ownerName = "Orlando OFF 🤖"; // Nombre del dueño
